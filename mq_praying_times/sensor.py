...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    if unique_id not in hass.data[DOMAIN]:
        # Initialize a place in hass.data[DOMAIN] if it doesn't exist
        hass.data[DOMAIN][unique_id] = []

    # Check if the entry has already been setup
    if hass.data[DOMAIN][unique_id]:
        # The entities are already setup for this config_entry
        return False
...
class PrayerTimeSensor(Entity):
    ...
    async def async_added_to_hass(self):
        # Register a callback for when the configuration is updated
    ...
    def _handle_config_update(self, new_config):
        ...
        # Call the entity update method to apply new configuration
        self.async_schedule_update_ha_state(True)

    async def async_update(self):
        """Fetch new state data for the sensor."""
        # Retrieve user configuration from config entry
...
def calculate_prayer_times(zona, lintang, bujur, ketinggian, sudut_subuh, sudut_dhuha, sudut_isya, ikhtiyat_subuh, ikhtiyat_zuhur, ikhtiyat_ashar, ikhtiyat_maghrib, ikhtiyat_isya):
    ...
    tgl = int(current_time.strftime('%-d'))  # Day of the month
    bln = int(current_time.strftime('%-m'))  # Month
    thn = int(current_time.strftime('%Y'))  # Year
    ...
    subuh = ikhtiyat_subuh / 60 + TransitM - (12 / math.pi) * math.acos((math.sin(-1 * sudut_subuh * math.pi / 180) - math.sin(DeklinasiRad) * math.sin(LintangRad)) / (math.cos(DeklinasiRad) * math.cos(LintangRad)))
    dhuha = TransitM - (12 / math.pi) * math.acos((math.sin(sudut_dhuha * math.pi / 180) - math.sin(DeklinasiRad) * math.sin(LintangRad))/(math.cos(DeklinasiRad) * math.cos(LintangRad)))
    zuhur = ikhtiyat_zuhur / 60 + TransitM
    ashar = ikhtiyat_ashar / 60 + TransitM + (12 / math.pi) * math.acos((math.sin(math.atan(1 / (1 + math.tan(abs(LintangRad - DeklinasiRad)))) - math.sin(DeklinasiRad) * math.sin(LintangRad)) / (math.cos(DeklinasiRad) * math.cos(LintangRad))))
    maghrib = ikhtiyat_maghrib / 60 + TransitM + (12 / math.pi) * math.acos((math.sin((-0.8333 - 0.0347 * ketinggian ** 0.5) * math.pi / 180) - math.sin(DeklinasiRad) * math.sin(LintangRad)) / (math.cos(DeklinasiRad) * math.cos(LintangRad)))
    isya = ikhtiyat_isya / 60 + TransitM + (12 / math.pi) * math.acos((math.sin(-1 * sudut_isya * math.pi / 180) - math.sin(DeklinasiRad) * math.sin(LintangRad))/ (math.cos(DeklinasiRad) * math.cos(LintangRad)))
    imsak = subuh - 10 / 60
